    ...
    private List<List<CoreLabel>> postProcess(List<CoreLabel> question, List<CoreLabel> statement) {
        ...
        for (CoreLabel token : statement) {
            ...
            switch (originalText.toLowerCase()) {
                case "i":
                    token.set(StatementTextAnnotation.class, "you");
                    break;
                case "you":
                    token.set(StatementTextAnnotation.class, "i");
                    break;
                case "my":
                    token.set(StatementTextAnnotation.class, "your");
                    break;
                case "your":
                    token.set(StatementTextAnnotation.class, "my");
                    break;
                default:
                    token.set(StatementTextAnnotation.class, originalText);
                    break;
            }
        }
        ...
        for (int i = 0; i < statement.size(); ++i) {
            ...
            String originalText = token.get(StatementTextAnnotation.class);
            String uppercase = originalText.length() == 0
                    ? originalText
                    : Character.toUpperCase(originalText.charAt(0)) + originalText.substring(1);
            if (i == 0) {
                token.set(StatementTextAnnotation.class, uppercase);
                ...
                switch (originalText.toLowerCase()) {
                    case "i":
                        ...
                        break;
                    default:
                        token.set(StatementTextAnnotation.class, originalText.toLowerCase());
                        break;
                }
            }
        }
        ...
        PERSON_LOOP: for (CoreLabel token : statement) {
            ...
            switch (token.get(StatementTextAnnotation.class).toLowerCase()) {
                case "us":
                    plural = true;
                    person = 1;
                    break PERSON_LOOP;
        ...
        for (CoreLabel token : statement) {
            if (Optional.ofNullable(token.tag()).map(x -> x.startsWith("V") && !x.equals("VBG") && !"be".equals(token.word())).orElse(false)) {
                foundVerb = true;
                token.set(StatementTextAnnotation.class,
                        tense.conjugateEnglish(token.get(StatementTextAnnotation.class), false));
            }
        }
        if (!foundVerb) {
            for (CoreLabel token : statement) {
